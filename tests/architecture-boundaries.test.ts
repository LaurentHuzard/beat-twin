...
import { validateArchitecture } from "../scripts/architecture-boundaries.ts";
...
test("legacy app imports are exact and never authorize a package-to-app import", () => {
  withFixture([
    workspace(".", "fixture-root"),
    workspace("apps/operator", "@beat-twin/operator", [], "import '../../../index.js'; import '../../../other.js';"),
    workspace("packages/domain", "@beat-twin/domain", [], "import '../../index.js';"),
  ], (root) => {
    const policy = { ...BASE_POLICY, relativeAppImports: [
      { from: "apps/operator/src/index.js", to: "index.js", reason: "Legacy client" },
      { from: "packages/domain/src/index.js", to: "packages/index.js", reason: "Must not bypass package boundary" },
    ] };
    const errors = validateArchitecture(root, policy).errors;
    assert.equal(errors.length, 2);
    assert.ok(errors.some((error) => error.includes("other.js")));
    assert.ok(errors.some((error) => error.includes("@beat-twin/domain")));
  });
});
