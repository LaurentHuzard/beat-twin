...
Status: BT-ARCH-101 through 104 implemented locally; parent review and merge pending
...
The user authorized completing the existing PR #50/#51 work on main 86679ab.
BT-ARCH-101 through 104 are implemented in finish/beat-pending. Further slices
105 through 109 and the remaining issue #65 product roadmap are deferred.
See [the port report](../.agents/reports/architecture-20260912-port.md).
The decision and activation table below record the historical September 6
baseline; they do not supersede this current delivery status.

## Historical Revalidation Decision

Keep the modular-monolith direction and preserve the original dependency
order, but do not start a new refactor from this document. Draft PR #51 already
contains an old implementation attempt for `BT-ARCH-101` through
`BT-ARCH-104`; it must first be rebased, tested, and judged against current
code. Until that review finishes, every implementation row below remains
`proposed`, not `Done`.

Current relevance by slice:

| Slice | 2026-09-06 decision | Reason |
| --- | --- | --- |
| `BT-ARCH-101` | retain first | `main` still has no executable workspace dependency policy |
| `BT-ARCH-102` | retain second | MCP still imports the JavaScript Gateway app through an ambient shim |
| `BT-ARCH-103` | retain third | MCP package still owns full process composition and lifecycle |
| `BT-ARCH-104` | retain fourth, narrow claim | bounded process-lifetime retention is useful, but does not prove restart durability |
| `BT-ARCH-105` | defer | the 1,390-line browser store remains a hotspot, but current behavior is more valuable than a speculative split |
| `BT-ARCH-106` | defer | live audio orchestration remains timing-critical and needs dedicated characterization before movement |
| `BT-ARCH-107` | defer | the 57-tool compatibility facade remains in use and now shares controller concerns with newer Bitwig paths |
| `BT-ARCH-108` | reassess after 102/103 | manual build ordering remains, but graph changes should settle first |
| `BT-ARCH-109` | parked | only NanoDAW and Bitwig adapters exist; no third adapter trigger exists |

The roadmap therefore prepares reviewable future slices; it does not authorize
merging #51 or adding more implementation to that branch.
...
| ---: | --- | --- | --- | --- | --- | --- | --- | --- |
| 0 | BT-ARCH-100 | [#45](https://github.com/LaurentHuzard/beat-twin/issues/45) | Audit, ADR, roadmap, and documentation alignment | shared baseline | low | M | - | revalidated in draft #50 |
| 1 | BT-ARCH-101 | [#46](https://github.com/LaurentHuzard/beat-twin/issues/46) | Enforce workspace dependency direction in CI | prevents new inversions | low | S | BT-ARCH-100 | review existing draft #51 first |
...
- [ ] CI remains deterministic on Node 26 when GitHub Actions is enabled.
