...
Status: historical audit reconciled with the 2026-09-12 local refactor

Original baseline: `main` at `2ecd6cb7681c27ae04ba41519ec8ca50f9f1f282`

Date: 2026-07-20

Revalidated: 2026-09-06 against `main` at
`f4095ca3062e33bb86a1d412c94ab7ca88bc9b73`

## Current implementation — 2026-09-12

BT-ARCH-101 through 104 are ported onto main 86679ab in finish/beat-pending.
The dependency guard, gateway-http package, explicit apps/nanodaw-mcp
composition and bounded process-lifetime retention are implemented locally.
The current Gateway handler was extracted, preserving dual-target routes,
previewOnly, authentication and browser ownership. The old package-to-app
MCP dependency and ambient shim are removed. Two exact RTX CLI imports of
the root protocol client remain documented app-to-app compatibility edges.
Public Gateway declarations remain handwritten alongside JavaScript delivery;
compiling consumers checks these declarations, not the JavaScript implementation.
Protocol tests supply behavioral parity evidence. No restart durability is claimed.
See [the port report](../.agents/reports/architecture-20260912-port.md) for
verification and publication gates. The sections below are dated pre-refactor
findings retained as rationale rather than current inventory.

## Historical September 6 Revalidation

The architectural verdict and the first four migration boundaries remain
relevant. None of `BT-ARCH-101` through `BT-ARCH-104` has landed on `main`:

- `packages/mcp` still declares `@beat-twin/gateway` and imports its untyped
  JavaScript delivery API through `packages/mcp/src/gateway.d.ts`;
- `packages/mcp/src/runtime.ts` still composes HTTP, WebSocket, pairing, plan
  storage, the NanoDAW adapter, MCP stdio, startup, and shutdown;
- the root scripts expose no `check:architecture` command and no executable
  dependency-layer policy;
- Gateway pairings, plans, confirmations, execution reports, and adapter
  idempotency evidence are still process-memory state without restart recovery
  or a repository-wide bounded-retention contract.

The codebase has nevertheless advanced materially since July. The revalidated
inventory is 12 workspace manifests (13 including the root manifest), 87
TypeScript/TSX/JavaScript files under `apps/` and `packages/`, 42 test files,
and 96 Markdown documents. Merged work added browser-owned audio asset
references, first-run and shortcut UX, the secure Bitwig web remote, bounded
RTX proposal loops, one explicitly confirmed Bitwig write with honest partial
then read-only reconciliation, and preview-only Bitwig and dual-target runtime
compositions.

Those additions strengthen rather than invalidate the incremental approach:

- the browser remains the sole owner of NanoDAW `Song` state;
- the live write did not weaken fixed-target confirmation or no-mutation-retry
  rules;
- `apps/gateway` now contains additional RTX composition modules, making an
  explicit typed composition boundary more valuable;
- the largest current hotspots remain substantial: root `index.js` (1,537
  lines), browser `store.ts` (1,390), `liveAudioController.ts` (1,017), Gateway
  HTTP `index.js` (908), and Gateway core `index.ts` (876).

The implementation branch in draft PR #51 is useful design evidence, not
merged architecture. It must be reviewed as an old prototype on top of this
revalidated baseline; its existence does not mark any roadmap slice complete.
No new broad refactor is authorized by this audit.
...
The original audit covered all 12 workspace manifests (13 including the root
manifest), 74 TypeScript/TSX/JavaScript source files under `apps/` and
`packages/`, the root Bitwig MCP server, the Bitwig controller script, 38 test
files, 42 Markdown documents, CI, package build configuration, and the Orbit
execution contract. The September inventory above records the current scale;
the detailed findings below retain their July evidence and are superseded
where the revalidation says otherwise.
