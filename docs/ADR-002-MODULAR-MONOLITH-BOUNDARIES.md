...
Status: implementation authorized on 2026-09-12; local port awaiting parent review

Date: 2026-07-20

Revalidated: 2026-09-06 against `main` at
`f4095ca3062e33bb86a1d412c94ab7ca88bc9b73`. The dependency inversion,
composition ownership, and process-memory retention findings still apply.
Recent browser, Bitwig, and RTX additions do not require a rewrite or separate
services. Acceptance remains a human decision on draft PR #50; draft PR #51 is
implementation evidence, not acceptance of this ADR.

## Current port

Implemented BT-ARCH-101 through 104 on main 86679ab. The reusable MCP
package no longer imports an app. Gateway delivery has a public declaration
file and retains the current JavaScript handler with protocol regression tests;
this is not a claim that its implementation is fully TypeScript-checked.
Two current RTX CLI-to-root protocol imports are exact app-only exceptions
in architecture-policy.json, pending the separately deferred BT-ARCH-107.
The revalidation above and rationale below describe the historical baseline.
...
Gateway HTTP and browser WebSocket delivery now live behind a public package
API. The NanoDAW MCP application compiles against its declaration file; Gateway
RTX composition runs TypeScript source directly. The obsolete ambient
`packages/mcp/src/gateway.d.ts` shim is removed. Route tests protect JavaScript
behavior independently of the handwritten public declarations.
