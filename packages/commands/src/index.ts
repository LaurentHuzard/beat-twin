...
export type BeatTwinCommand =
  | { readonly type: "RenameTrack"; readonly trackId: string; readonly name: string }
  | { readonly type: "DeleteTrack"; readonly trackId: string }
  | { readonly type: "UpdateClip"; readonly trackId: string; readonly clipId: string; readonly name?: string; readonly startBeat?: number; readonly lengthBeats?: number }
  | { readonly type: "DeleteClip"; readonly trackId: string; readonly clipId: string }
  // Only generated by revision-bound recovery, never a model-facing arbitrary snapshot setter.
  | { readonly type: "RestoreTrack"; readonly track: Track; readonly index: number }
...
export type CommandEvent =
  | { readonly type: "TrackRenamed" | "TrackDeleted" | "TrackRestored"; readonly trackId: string }
  | { readonly type: "ClipUpdated" | "ClipDeleted"; readonly trackId: string; readonly clipId: string }
...
function applyCommand(
  ...
  switch (command.type) {
    ...
    case "RestoreTrack": {
      ...
      const trackId = command.type === "RestoreTrack" ? command.track.id : command.trackId;
      const index = tracks.findIndex((track) => track.id === trackId);
      if (command.type === "RestoreTrack") {
        if (!Number.isInteger(command.index) || command.index < 0 || command.index > tracks.length) throw new Error("Invalid track restore position");
        // Full graph validation rejects normalization and malformed recovery payloads.
        const validation = { song: { ...song, tracks: [command.track] }, revision: state.revision };
        if (!validateCommandSnapshot(validation)) throw new Error("Invalid restored track");
        ...
        if (command.type === "DeleteTrack") tracks.splice(index, 1);
        else if (command.type === "RenameTrack") {
          if (typeof command.name !== "string" || !command.name.trim()) throw new Error("Track name is required");
          tracks[index] = { ...track, name: command.name };
        } else {
          const clipIndex = track.clips.findIndex((clip) => clip.id === command.clipId);
          if (clipIndex < 0) throw new Error(`Clip not found: ${command.clipId}`);
          const clips = [...track.clips];
          if (command.type === "DeleteClip") clips.splice(clipIndex, 1);
          else {
            const clip = clips[clipIndex]!;
            if (command.name !== undefined && (typeof command.name !== "string" || !command.name.trim())) throw new Error("Clip name is required");
            const lengthBeats = command.lengthBeats ?? clip.lengthBeats;
            if (clip.pattern.notes.some((note) => note.startBeat + note.lengthBeats > lengthBeats)) throw new Error("Clip resize would truncate notes; edit notes explicitly first");
            clips[clipIndex] = createClip({ ...clip, name: command.name ?? clip.name, startBeat: command.startBeat ?? clip.startBeat, lengthBeats, pattern: { ...clip.pattern, lengthBeats } });
          }
          tracks[index] = { ...track, clips };
        }
      }
      const nextSong = deserializeSong({ ...song, tracks });
      const type = command.type === "RenameTrack" ? "TrackRenamed" : command.type === "DeleteTrack" ? "TrackDeleted" : command.type === "RestoreTrack" ? "TrackRestored" : command.type === "UpdateClip" ? "ClipUpdated" : "ClipDeleted";
      const event = { type, trackId, ...("clipId" in command ? { clipId: command.clipId } : {}) } as CommandEvent;
      return freezeState({ ...state, song: nextSong, selection: { type: "song", id: song.id }, log: [...state.log, event] });
    }
    case "CreateSong": {
