...
test("successful readback with exhausted observation storage remains uncertain and cannot replay", async () => {
  const port = new MemoryBitwigPort();
  const instance = adapter(port, {
    observationRetention: { capacity: 1 },
    executionRetention: { capacity: 1 },
  });
  const snapshot = await instance.inspect();
  const request = plan(snapshot.commandSnapshot.revision);
  const report = await instance.execute(request);
  assert.equal(report.status, "partial");
  assert.match(report.ok ? "" : report.error.message, /readback succeeded but retention failed/);
  const mutations = port.calls.length;
  assert.ok(mutations > 0);
  assert.equal(await instance.execute(request), report);
  assert.equal(port.calls.length, mutations);
  const blocked = await instance.execute(plan(snapshot.commandSnapshot.revision, { requestId: "another" }));
