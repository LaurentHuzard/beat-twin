...
export function validateArchitecture(rootDir, policy = readPolicy(rootDir)) {
  ...
  for (const workspace of workspaces) {
    ...
    validateRuntimeImports(rootDir, workspace, byName, errors, policy);
  }
...
function validateRuntimeImports(rootDir, workspace, byName, errors, policy) {
    ...
    for (const specifier of importSpecifiers(source)) {
      if (specifier.startsWith(".")) {
        ...
        if (targetWorkspace && targetWorkspace.name !== workspace.name) {
          // Existing RTX CLI composition uses the root application's protocol
          // client. This exact app-to-app edge does not authorize package imports.
          const allowed = workspace.layer === "app" && targetWorkspace.layer === "app" &&
            (policy.relativeAppImports ?? []).some((entry) =>
              entry.from === toPosix(relative(rootDir, file)) &&
              entry.to === toPosix(relative(rootDir, targetPath)) &&
